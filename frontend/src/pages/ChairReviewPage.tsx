...
import type {
  ChairRequestActionDTO, ChairReviewDTO,
...
export default function ChairReviewPage() {
  ...
  async function handleApprove() {
    ...
    try {
      const action =
        await ChairReviewRequestService.approve(
          Number(requestId),
          {
            points: parsedPoints,
  ...
  async function handleFinalReject() {
    ...
    if (!normalizedFeedback) {
      setActionError(
        'Feedback is required when rejecting evidence.'
      )
      return
    }
    ...
    try {
      const action =
        await ChairReviewRequestService.reject(
          Number(requestId),
  ...
  useEffect(() => {
    ...
    ChairReviewRequestService
      .getRequestForReview(Number(requestId))
      .then((data) => {
        if (cancelled) return;
              ...
              {steps.map((label, idx) => {
                ...
                return (
                  <div key={label} className="flex flex-col items-center z-10" style={{ width: `${100 / steps.length}%` }}>
                    <div
                      className={`flex items-center justify-center w-9 h-9 rounded-full border-2 ${isDone
